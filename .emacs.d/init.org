...
I prefer having my files save automatically. Any changes I don't want, I just don't commit to git.
...
=Olivetti= is a minor mode that let's us center our text.
I want to be able to activate it using the keybinding =C-c C-o=.
...
I still have a lot to learn about these modes, but I've done my best to prettify the way =.org=-files are displayed in my config. First, I'll modify the typefaces and their relative sizes.
...
Next, I want to center the text, and enable linebreaks. But I only want to do this for Org mode, so I'll make it a hook so it's only activated when I enter Org mode.
